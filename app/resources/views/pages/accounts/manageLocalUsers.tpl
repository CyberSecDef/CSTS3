      ...
      <div>
          <div class="alert alert-secondary border-secondary" role="alert">
            ...
            <p>
              ...
              users are present on selected computers, and what their account properties are.  
        ...
        <div class="card">
          ...
          <div id="accounts-manageLocalUsers-results" class="collapse " aria-labelledby="headingTwo" data-parent="#accordion">
            ...
            <div style="display:block;margin-bottom:60px;">
                ...
                <form class="form-inline float-left" style="margin-left:20px;">
                    <div class="form-group mb-2">
                      <div class="btn-group" role="group">
                        <button class="btn btn-outline-success enableAccount" type="button" data-table="accounts-manageLocalUsers-results-tbl">
                          Enable
                        </button>
                        <button class="btn btn-outline-warning disableAccount" type="button" data-table="accounts-manageLocalUsers-results-tbl">
                          Disable
                        </button>
                        <button class="btn btn-outline-danger deleteAccount" type="button" data-table="accounts-manageLocalUsers-results-tbl">
                          Delete
                        </button>
                      </div>
                    </div>
              ...
              <form class="form-inline float-right" style="margin-right:20px;" >
                <div class="form-group mb-2">
                  <div class="btn-group" role="group">
...
<script>
  $(document).ready(function() {
    $('table#accounts-manageLocalUsers-results-tbl tbody tr').on( 'click', () => {
  ...
  $('div#accounts-manageLocalUsers-results form button.exportDOC').on('click',function(){
    $results = $('<table></table>')
    $row = $('<tr></tr>');
    $.each( $("table#" + $(this).data('table') + " thead tr th").not('th:first') , function(d,el){
      $row.append($('<th></th>').text( $(el).text() ));
    });
    $results.append($row);
    $.each( $("table#" + $(this).data('table') + " tbody tr") , function(d,el){
      $row =  $('<tr></tr>');
      $.each( $(el).find('td').not('td:first'), (dd, e) => {
        $row.append($('<td></td>').text($(e).text()));
      })
      $results.append($row);
		})
  ...
  $('div#accounts-manageLocalUsers-results form button.exportCSV').on('click',function(){
    data = {};
		data.columns = $("th",$("table#" + $(this).data('table') + " thead")).not('th:first').map(function() {  return (this.innerText || this.textContent) }).get();
    data.rows = [];
		$.each( $("table#" + $(this).data('table') + " tbody tr"), function(d,el){
      const row = [];
      $.each( $(el).find('td').not('td:first'), (dd, e) => {
  ...
  $('div#accounts-manageLocalUsers-results form button.exportPDF').on('click',function(){
    data = {};
    data.styles = {
			columnStyles : { },
  ...
  $('div#accounts-manageLocalUsers-results form button.enableAccount').on('click',function(){
    // $u = [ADSI]"WinNT://wsz3898-b0i/b30_admin,user"
    // $u.AccountDisabled = $true
    // $u.setinfo()
    $.each( $('table#' + $(this).data('table')).find('input:checked'), (i,c) => { console.log(atob( $(c).val()) ) })
  });
